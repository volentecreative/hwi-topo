...
<script src="https://cdn.jsdelivr.net/gh/volentecreative/hwi-topo@3e6b3db/topo-turntable.js"></script>
<script>
...
> The URL above is pinned to commit `3e6b3db`, so it is permanent and served instantly. Swap the hash for a newer commit to pick up changes; `@main` also works but jsDelivr caches it for up to 24 h. The script loads its terrain from `data/` beside itself, so the pin covers the data too.
